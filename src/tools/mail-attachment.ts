...
export function registerMailAttachmentTools(server: McpServer, config: Config) {
  ...
  server.tool(
    ...
    "Download an email attachment and return its content. Use extract_text=true to get the text content of PDF, DOCX, XLSX, PPTX and other document formats directly.",
    {
      ...
      filename: z.string().describe("Attachment filename to download (from read_email attachments list)"),
      extract_text: z.boolean().default(true).describe("Extract text content from document (PDF, DOCX, XLSX, PPTX, TXT, HTML) or image/scan (JPEG, PNG, TIFF via VLM-OCR). If false, returns raw base64."),
      prompt: z.string().optional().describe("Optional: custom question for images/scans (e.g. 'Was kostet die Rechnung?'). Only used for image attachments. Default: extract all text."),
    },
    async ({ account: accountName, uid, folder, filename, extract_text, prompt }) => {
...
async function extractText(bytes: Buffer, filename: string, mimeType: string, prompt?: string): Promise<string> {
  ...
  if (mimeType.includes("text/html") || ext === "html" || ext === "htm") {
    return bytes.toString("utf-8").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
  }

  // Images/scans — use VLM (OCR)
  ...
  try {
    ...
    // DOCX, XLSX, PPTX, ODT, ODS, ODP, RTF — LibreOffice
    const libreofficeFormats = ["docx", "doc", "xlsx", "xls", "pptx", "ppt", "odt", "ods", "odp", "rtf"];
    if (libreofficeFormats.includes(ext)) {
      try {
        await execFileAsync("libreoffice", [
          "--headless", "--convert-to", "txt:Text", "--outdir", tmpDir, inFile,
        ]);
...
async function analyzeImageWithVLM(bytes: Buffer, mimeType: string, prompt?: string): Promise<string> {
  const base64 = bytes.toString("base64");
  const dataUrl = `data:${mimeType};base64,${base64}`;
  const question = prompt ?? "Lies alle Texte, Zahlen und wichtigen Inhalte aus diesem Bild/Scan vor. Falls es ein Dokument oder eine Rechnung ist, gib alle Felder mit ihren Werten aus.";
  ...
  return new Promise((resolve) => {
    ...
    const req = request(options, (res: import("node:http").IncomingMessage) => {
      let data = "";
